...
# --------------------------------------------------------
# Use the write() method of the PySpark DataFrameWriter object to write PySpark DataFrame to a CSV fil
#  Options:  While writing a CSV file you can use several options. for example, header to output the DataFrame column
...
df3.write.mode('overwrite').csv("spark_output/zipcodes")   # overwrite – mode is used to overwrite the existing file
# one can also use this
df2.write.format("csv").mode('overwrite').save("output/zipcodes")